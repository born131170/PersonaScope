...
export function VideoUpload() {
  ...
  const handleFile = useCallback((file: File) => {
    if (!file.type.startsWith('video/')) {
      setError('Пожалуйста, загрузите видеофайл (MP4, WebM, MOV, AVI)');
      return;
    }
    setError(null);
  ...
  const handleVideoLoaded = () => {
    if (videoRef.current && video) setVideo({ ...video, duration: videoRef.current.duration });
  };
  ...
  const runAnalysis = async () => {
    ...
    const steps = [
      ...
      'Анализ языка тела...',
      'Обработка вокальных паттернов...',
      'Расчёт профилей личности...',
      'Оценка правдивости...',
      'Создание цифровых слепков...',
    ];
    ...
    if (settings.apiKey) {
      try {
        await callLLM(settings, [
          ...
          { role: 'user', content: `Проанализируй загруженное видео: ${video.name}. Длительность: ${video.duration}с. Проведи комплексный анализ личности.` },
        ]);
      } catch (err) {
        console.warn('LLM вызов не удался, используется локальный движок:', err);
      }
    }
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        ...
        <p className={isDark ? 'text-gray-400' : 'text-gray-600'}>
          Загрузите видео для комплексного анализа личности с помощью мультимодального ИИ.
        </p>
      </div>
        ...
        <div
          className={`relative border-2 border-dashed rounded-2xl p-16 text-center transition-all duration-300 ${
            dragActive
              ...
              : isDark ? 'border-gray-700 hover:border-gray-600 bg-gray-900/30' : 'border-gray-300 hover:border-gray-400 bg-gray-100/50'
          }`}
        ...
        >
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <p className={`text-sm mt-1 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>или нажмите для выбора файла</p>
            </div>
            <button onClick={() => fileInputRef.current?.click()}
              className="px-6 py-3 rounded-xl bg-violet-600 hover:bg-violet-500 text-white font-medium transition-colors">
              Выбрать видеофайл
            </button>
            <p className={`text-xs ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>MP4, WebM, MOV, AVI • Макс. 500МБ</p>
          </div>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            <button onClick={runAnalysis} disabled={isAnalyzing}
              className="flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 text-white font-medium transition-all disabled:opacity-50">
              {isAnalyzing ? (<><Loader2 className="w-5 h-5 animate-spin" />Анализ...</>) : (<><Play className="w-5 h-5" />Запустить анализ</>)}
            </button>
            <button onClick={() => { setVideo(null); setError(null); }}
          ...
          {isAnalyzing && (
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Прогресс анализа</span>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                {['Извлечение кадров', 'Лицевой анализ', 'Микро-выражения', 'Язык тела', 'Вокальные паттерны', 'Профили личности'].map((step, i) => (
                  <div key={step} className={progress > (i + 1) * 16 ? 'text-emerald-500' : isDark ? 'text-gray-600' : 'text-gray-400'}>
                    {progress > (i + 1) * 16 ? '✓' : '○'} {step}
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { title: '6 систем', desc: 'Big Five, MBTI, Эннеаграмма, Темперамент, HEXACO, PID-5' },
          { title: 'Анализ кадров', desc: 'Микро-выражения, жесты, движение глаз, поза' },
          { title: 'Цифровой слепок', desc: 'Уникальная поведенческая сигнатура для поиска' },
        ].map((card) => (
          <div key={card.title} className={`p-4 rounded-xl border ${isDark ? 'bg-gray-900/50 border-gray-800' : 'bg-white border-gray-200'}`}>
            <h4 className="text-sm font-semibold text-violet-500 mb-1">{card.title}</h4>
            <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>{card.desc}</p>
          </div>
        ))}
      </div>
    </div>
