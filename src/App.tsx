  ...
  return (
    <div className={`flex h-screen overflow-hidden ${theme === 'dark' ? 'bg-gray-950 text-gray-100' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <main className="flex-1 overflow-y-auto">
        <header className={`sticky top-0 z-10 backdrop-blur-xl border-b px-8 py-4 ${theme === 'dark' ? 'bg-gray-950/80 border-gray-800' : 'bg-white/80 border-gray-200'}`}>
          <div className="flex items-center gap-3">
            ...
            {video && <span className="ml-auto text-sm text-gray-500 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"/>{video.name}</span>}
            {analysis && <span className="ml-2 text-xs px-2 py-1 rounded-full bg-violet-500/20 text-violet-500 border border-violet-500/30">Анализ завершён</span>}
            <button onClick={toggleTheme} className={`ml-4 w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${theme === 'dark' ? 'bg-gray-800 hover:bg-gray-700 text-yellow-400' : 'bg-gray-200 hover:bg-gray-300 text-gray-700'}`} title={theme === 'dark' ? 'Светлая тема' : 'Тёмная тема'}>
